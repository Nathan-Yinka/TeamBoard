...
import { Plus, Settings, LogOut, LayoutDashboard, Search, Filter, SortDesc, Activity } from 'lucide-react';
  ...
  return (
    <main className="flex h-screen w-full bg-slate-50 text-slate-900 overflow-hidden font-sans">
      ...
      <section className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50/50">
        <header className="bg-white border-b border-slate-200 px-8 py-6 flex flex-col gap-6 shrink-0 shadow-sm relative z-0">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs font-bold text-teal-600 uppercase tracking-wider mb-1">Project</p>
            ...
            <div className="flex items-center gap-3">
              {selectedProjectId && (
                <RadixButton variant="soft" color="gray" size="3" style={{ cursor: 'pointer' }} onClick={() => setIsAuditTrailOpen(true)}>
                  <Activity size={16} /> Activity
                </RadixButton>
              )}
              <Dialog.Root open={isCreateTaskOpen} onOpenChange={setIsCreateTaskOpen}>
                <Dialog.Trigger>
                  <RadixButton disabled={!selectedProjectId} size="3" style={{ cursor: 'pointer' }}>
